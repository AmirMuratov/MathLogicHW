...
sealed trait FAExpression {
  ...
  def substitute(varToSubst: String, subst: Term): FAExpression = {
    this match {
      ...
      case a@AnySubst(name, expr) => if (name.equals(varToSubst)) a else
        AnySubst(name, expr.substitute(varToSubst, subst))
      case a@ExistsSubst(name, expr) => if (name.equals(varToSubst)) a else
  ...
  def getVariables: Set[String] = {
    ...
    case Predicate(name, vars) => vars.flatMap(_.getVariables)
