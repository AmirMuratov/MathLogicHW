        ...
        if (FAAxioms.check(e).isDefined || newHypothesis.contains(e)) {
          ...
          e match {
            case all@Implication(l, an@AnySubst(x, r)) =>
              if (vars.contains(x))
                throw new UsageOfVarThatEntersHyp(exp._2, "правило", x, a)
          ...
          if (mp.isDefined) {
            ...
            throw new WrongInference(exp._2, exp._1)
          }
        }
      } catch {
        case e: TermNotFreeForSubst => e.setN(exp._2); throw e
        case e: VarEnterFree => e.setN(exp._2); throw e
      }
